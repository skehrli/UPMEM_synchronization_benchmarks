...
do
    ...
    do
        ...
        for ((tasklets = 1; tasklets <= 200; tasklets*=2))
        do
            #Run the benchmark
            command="bench lock_kernel ${tasklets} ${numOps} ${critical_section[$cs]} ${main_section[$ms]}"
            command_nosync="bench lock_kernel_nosync ${tasklets} ${numOps} ${critical_section[$cs]} ${main_section[$ms]}"
            if [ -z ${1+x} ]; 
            then 
